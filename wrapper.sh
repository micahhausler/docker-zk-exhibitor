...
DEFAULT_AUTO_MANAGE_SETTLING_PERIOD=0
S3_SECURITY=""
HTTP_PROXY=""
...
: ${AUTO_MANAGE_SETTLING_PERIOD:=$DEFAULT_AUTO_MANAGE_SETTLING_PERIOD}
: ${HTTP_PROXY_HOST:=""}
: ${HTTP_PROXY_PORT:=""}
: ${HTTP_PROXY_USERNAME:=""}
: ${HTTP_PROXY_PASSWORD:=""}
...
cat <<- EOF > /opt/exhibitor/defaults.conf
	...
	auto-manage-instances-settling-period-ms=$AUTO_MANAGE_SETTLING_PERIOD
